...
Можем ограничить определение утечки: ошибочная попытка удаления значения,
которое уже недоступно. Rust не борется с ней. На самом деле у Rust
даже есть функция *для нее*: `mem::forget`. Эта функция съедает полученное
значение *и не вызывает его деструктор*.

Раньше `mem::forget` помечалась unsafe для ее статической проверки, из-за того,
...
Разумно в безопасном коде предполагать, что утечка в деструкторе не происходит,
потому что любая программа с такой утечкой неправильна. Однако *небезопасный*
код не может полагаться, на то, что вызов деструкторов является безопасными. Для
большинства типов это роли не играет: если утечка в деструкторе, то тип по
определению недоступен, поэтому это и не важно, не так ли? Например, если утечка
в `Box<u8>`, то вы тратите память впустую, но это вряд ли нарушит безопасность
памяти.
...
Вместо этого, мы можем захотеть, чтобы Drain восстанавливал то, что осталось в
Vec после своего удаления. Он должен закончить выполнение, восстановить любые
элементы, которые еще не были удалены (истощить вспомогательные поддиапазоны),
