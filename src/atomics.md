...
мы можем извлечь выгоду из существующих инструментов и исследовать Си.
...
программы и чтобы они делали то, что мы написали, и побыстрее. Разве это так
много?
...
может сделать так, что некоторые события никогда не произойдут! Если мы напишем,
что-нибудь такое
...
если наша программа многопоточна, мы рассчитываем, что `x` присвоется 1 до
...
Проблемы придут от CPU из-за иерархий памяти. Есть единственное глобальное общее
пространство памяти где-то в вашей аппаратной части, но с точки зрения каждого
ядра CPU оно *так далеко* и *так медленно*. Каждый CPU лучше будет работать со
...
всего* выполнится, даже если ваша программа категорически неправильна. Если это
...
Модель памяти C11 пытается наводить мосты, позволяя говорить нам о *причинно-
следственной связи* нашей программы. Как правило, устанавливаются *ранние*
связи между частями программы и потоками, выполняющими их. Это дает железу и
компилятору место для более агрессивной оптимизации программы, когда строгие
ранние отношения еще не установлены, но заставляет их быть очень аккуратными,
когда они уже установлены. Налаживаются отношения по средствам *обращения к данным*
и *атомарных доступов*.

Обращения к данным - это хлеб с маслом в мире программирования. Они
...
захочет. Самое опасное, что обращение к данным - это место для гонок данных.
...
**Это, буквально, невозможно, написать правильно синхронизированный код,
используя только обращения к данным.**
...
гарантируя что важные секции не пересекутся.
